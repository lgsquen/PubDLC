...
import xyz.pub.modules.settings.impl.PositionSetting;
...
public abstract class HudElement extends Module {
    ...
    public void onRender2D(EventRender2D e) {
        ...
        if (button) {
            if (!dragging && MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY())) {
                ...
                dragY = mouseY() - getY();
                dragging = true;
                Pub.getInstance().getHudManager().setCurrentDragging(this);
            }
    ...
    public void onRender2DX2(EventRender2D e) {
        ...
        toggledAnimation.update(Pub.getInstance().getHudManager().getElements().getName("elements.settings.elements." + getName().toLowerCase()).getValue());
    }
    ...
    public void onMouse(EventMouse e) {
        ...
        if (e.getAction() == 0) {
            button = false;
            dragging = false;
            Pub.getInstance().getHudManager().setCurrentDragging(null);
        } else if (e.getAction() == 1) {
            if (e.getButton() == 0 && MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY()) && Pub.getInstance().getHudManager().getCurrentDragging() == null)
                button = true;
            ...
            if (e.getButton() == 1) {
                if (MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY())) {
                    ...
                    if (settings.size() == 1) {
                        if (Pub.getInstance().getHudManager().getWindow() != null) Pub.getInstance().getHudManager().setWindow(null);
                        Pub.getInstance().getNotifyManager().add(new Notify(NotifyIcons.failIcon, "У этого элемента нету настроек!", 1000));
                    } else {
                        if (Pub.getInstance().getHudManager().getWindow() != null)
                            Pub.getInstance().getHudManager().getWindow().reset();
