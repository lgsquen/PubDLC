...
import xyz.pub.modules.settings.api.*;
...
import xyz.pub.utils.other.FileUtils;
...
public class ConfigManager {
    ...
    private final String extension = ".pub";
    ...
    public void saveConfig(String config) throws Exception {
        FileUtils.reset(Pub.getInstance().getConfigsDir() + "/" + config + extension);
        JsonObject object = new JsonObject();
        object.add("config", new JsonPrimitive(config));
        object.add("modules", serializeModules());
        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Pub.getInstance().getConfigsDir() + "/" + config + extension), StandardCharsets.UTF_8);
    ...
    public void loadConfig(String config) throws Exception {
        if (!FileUtils.exists(Pub.getInstance().getConfigsDir() + "/" + config + extension)) return;
        InputStream stream = Files.newInputStream(Paths.get(Pub.getInstance().getConfigsDir() + "/" + config + extension));
    ...
    public void saveGlobals() throws Exception {
        FileUtils.reset(Pub.getInstance().getGlobalsDir() + "/" + "globals" + extension);
        JsonObject object = new JsonObject();
        object.add("config", new JsonPrimitive(currentConfig));
        JsonArray friendsArray = new JsonArray();
        Pub.getInstance().getFriendManager().getFriends().forEach(friendsArray::add);
        object.add("friends", friendsArray);
        JsonArray macrosArray = new JsonArray();
        Pub.getInstance().getMacroManager().getMacros().forEach(macro -> macrosArray.add(macro.getName() + ":" + macro.getCommand() + ":" + macro.getBind().getKey()));
        object.add("macros", macrosArray);
        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Pub.getInstance().getGlobalsDir() + "/" + "globals" + extension), StandardCharsets.UTF_8);
        writer.write(new GsonBuilder().setPrettyPrinting().create().toJson(JsonParser.parseString(object.toString())));
    }

    public void loadGlobals() throws Exception {
        if (!FileUtils.exists(Pub.getInstance().getGlobalsDir() + "/" + "globals" + extension)) return;
        InputStream stream = Files.newInputStream(Paths.get(Pub.getInstance().getGlobalsDir() + "/" + "globals" + extension));
        JsonObject object = JsonParser.parseReader(new InputStreamReader(stream)).getAsJsonObject();
        ...
        if (object.has("friends")) {
            for (JsonElement element : object.get("friends").getAsJsonArray()) {
                if (Pub.getInstance().getFriendManager().isFriend(element.getAsString())) continue;
                Pub.getInstance().getFriendManager().add(element.getAsString());
            }
        }
        if (object.has("macros")) {
            for (JsonElement element : object.get("macros").getAsJsonArray()) {
                ...
                Pub.getInstance().getMacroManager().add(new Macro(name, command, bind));
            }
        }
    }
    ...
    private JsonObject serializeModules() {
        ...
        for (HudElement element : Pub.getInstance().getHudManager().getHudElements()) {
            JsonObject object = new JsonObject();
            JsonObject settings = new JsonObject();
