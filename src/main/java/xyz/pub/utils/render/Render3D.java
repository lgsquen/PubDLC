package xyz.pub.utils.render;

import xyz.pub.api.mixins.accessors.IWorldRenderer;
...
public class Render3D implements Wrapper {
    ...
    public static void drawSphere(MatrixStack matrixStack, Vec3d center, float radiusX, float radiusY, float radiusZ, int segments, Color color) {
        if (!rendering) return;

        Matrix4f matrix = matrixStack.peek().getPositionMatrix();
        Vec3d camera = mc.gameRenderer.getCamera().getPos();

        // Создаем сферу используя сферические координаты
        for (int i = 0; i < segments; i++) {
            for (int j = 0; j < segments; j++) {
                ...
                // Вычисляем вершины квада
                Vec3d v1 = getSphereVertex(center, radiusX, radiusY, radiusZ, theta1, phi1);
                Vec3d v2 = getSphereVertex(center, radiusX, radiusY, radiusZ, theta1, phi2);
                Vec3d v3 = getSphereVertex(center, radiusX, radiusY, radiusZ, theta2, phi2);
                Vec3d v4 = getSphereVertex(center, radiusX, radiusY, radiusZ, theta2, phi1);

                // Трансформируем относительно камеры
                v1 = v1.subtract(camera);
                v2 = v2.subtract(camera);
                v3 = v3.subtract(camera);
                v4 = v4.subtract(camera);

                // Добавляем квад в список для рендеринга
                QUADS.add(new VertexCollection(
                        new Vertex(matrix, (float) v1.x, (float) v1.y, (float) v1.z, color.getRGB()),
                        new Vertex(matrix, (float) v2.x, (float) v2.y, (float) v2.z, color.getRGB()),
                        new Vertex(matrix, (float) v3.x, (float) v3.y, (float) v3.z, color.getRGB()),
                        new Vertex(matrix, (float) v4.x, (float) v4.y, (float) v4.z, color.getRGB())
                ));
            }
        }
    }

    private static Vec3d getSphereVertex(Vec3d center, float radiusX, float radiusY, float radiusZ, float theta, float phi) {
        float x = (float) (center.x + radiusX * Math.sin(theta) * Math.cos(phi));
        float y = (float) (center.y + radiusY * Math.cos(theta));
        float z = (float) (center.z + radiusZ * Math.sin(theta) * Math.sin(phi));
    ...
    public static void drawLine(MatrixStack matrixStack, Vec3d start, Vec3d end, Color color) {
        ...
        Vec3d transformedStart = start.subtract(camera);
        Vec3d transformedEnd = end.subtract(camera);


        DEBUG_LINES.add(new VertexCollection(
                new Vertex(matrix, (float) transformedStart.x, (float) transformedStart.y, (float) transformedStart.z, color.getRGB()),
                new Vertex(matrix, (float) transformedEnd.x, (float) transformedEnd.y, (float) transformedEnd.z, color.getRGB())
        ));
    }
