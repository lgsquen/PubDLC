...
import xyz.pub.Pub;
import xyz.pub.api.mixins.accessors.IClientPlayerInteractionManager;
import xyz.pub.modules.impl.movement.GuiMove;
...
public class InventoryUtils implements Wrapper {
    ...
    public void switchSlot(Switch mode, int slot, int previousSlot) {
        if (slot == -1 || previousSlot == -1 || slot == Pub.getInstance().getServerManager().getServerSlot()) return;
    ...
    public void bypassSwap(int slot, int targetSlot) {
        ...
        if (Pub.getInstance().getModuleManager().getModule(GuiMove.class).funtime.getValue()) {
           	Pub.getInstance().getModuleManager().getModule(GuiMove.class).setTicks(8);
           	
        	new Thread(() -> {
                try {
                	if (Pub.getInstance().getModuleManager().getModule(GuiMove.class).funtime.getValue()) Thread.sleep(Pub.getInstance().getModuleManager().getModule(GuiMove.class).getTicks() * 50L);
