...
public class ClickGui extends Screen implements Wrapper {
    ...
    public ClickGui() {
        super(Text.of("pub-clickgui"));
        for (Category category : Pub.getInstance().getModuleManager().getCategories()) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (close && openAnimation.getValue() <= 0.01f) {
            Pub.getInstance().getModuleManager().getModule(UI.class).setToggled(false);
            super.close();
            return;
        }
