...
import xyz.pub.modules.impl.movement.ElytraForward;
...
import xyz.pub.utils.animations.infinity.RotationAnimation;
import xyz.pub.utils.combat.IdealHitUtils;
import xyz.pub.utils.combat.PredictUtils;
...
import xyz.pub.utils.world.MultipointUtils;
...
public class Aura extends Module {
    ...
    public void onPlayerTick(EventPlayerTick e) {
        ...
        if (target != null || !backTimer.passed(250)) {
            ...
            if (target != null) {
                ...
                if (Pub.getInstance().getModuleManager().getModule(ElytraForward.class).isToggled()) {
                    int predict = Pub.getInstance().getModuleManager().getModule(ElytraForward.class).forward.getValue().intValue();
            ...
            if (target != null) {
                if (rotate.getValue() == Rotate.Packet) Pub.getInstance().getRotationManager().addPacketRotation(currentRotations);
                else Pub.getInstance().getRotationManager().addRotation(changer);
    ...
    private void attack() {
        ...
        if (sprint.getValue() == Sprint.Hvh
                && Pub.getInstance().getServerManager().isServerSprinting()
