...
public class AutoVault extends Module {
    ...
    private void clickVault(BlockPos pos, BlockState state, VaultBlockEntity entity) {
        ...
        if (slot == -1) {
            mc.player.sendMessage(Text.of("[Pub] Slot with ominous is empty"), false);
            return;
        }
        mc.player.sendMessage(Text.of("[Pub] " + entity.getSharedData().getDisplayItem().getItem().getName().getString()), false);
